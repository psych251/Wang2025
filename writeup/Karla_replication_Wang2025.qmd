...
No expected differences other than not starting the scale at neutral. Instead, my scale starts on the left-most corner at -7.
...
# convert data frame
df_participants_z <- as.data.frame(my_participants_z_matrix)
...
# Extract only the model prediction data (excluding the 'model' column)
df_model_data <- df_model[,-1]

# Apply Z-score (scale) row-wise (across the 30 trials for each model)
df_model_z_matrix <- t(apply(df_model_data, 1, scale))

# Convert to a data frame, set model names as row names, and trial names as columns
df_model_z <- as.data.frame(df_model_z_matrix)
colnames(df_model_z) <- colnames(df_model_data)
rownames(df_model_z) <- df_model[,1] # Set row names to model names

# Transpose the Z-scored model data so trials are rows and models are columns
df_model_z_T <- as.data.frame(t(df_model_z))
df_model_z_T$trial <- rownames(df_model_z_T) # Move trial names to a column
rownames(df_model_z_T) <- NULL
...
my_correlation_data_z <- merge(df_human_z_mean, myversion, by = "trial")
...
# loop through each model to calculate correlation
for (i in 1:length(model_columns)) {
  model_name <- model_columns[i]
  
  # calculate the Pearson correlation between the human average Z-score and the current model Z-score
  r_value <- cor(my_correlation_data_z$human_average_z,
                 my_correlation_data_z[[model_name]],
                 method = "pearson")
  
  # save results
  my_correlation_results_z[i, "Pearson_Correlation_r"] <- r_value
}

# round numeric col
my_correlation_results_z$Pearson_Correlation_r <- round(my_correlation_results_z$Pearson_Correlation_r, 2)
...
for (i in 1:length(model_columns)) {
  ...
  # calculate the Pearson correlation between the human average Z-score and the current model Z-score
  r_value <- cor(df_correlation_data_z$human_average_z,
                 df_correlation_data_z[[model_name]],
                 method = "pearson")
  
  # save results
  correlation_results_z[i, "Pearson_Correlation_r"] <- r_value
}

# round numeric col
correlation_results_z$Pearson_Correlation_r <- round(correlation_results_z$Pearson_Correlation_r, 2)
