...
model_predictions <- read_csv("model predictions.csv")

#### Data exclusion / filtering
data <- raw_data %>% 
  filter(Finished == "TRUE") %>% 
  select(ResponseId, 21:50) %>% #### Prepare data for analysis - create columns etc.
  pivot_longer(!ResponseId, names_to = "question", values_to = "response") %>% 
  mutate(question = as.factor(question), response = as.numeric(response))
